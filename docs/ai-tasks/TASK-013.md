# TASK-013 — Create CI and audit static output
...
| Suggested commit | `task-013: create-ci-and-audit-static-output` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-013 only**. Do not start another task.

### Objective

Mirror local validation in GitHub Actions and prove output is static.
...
1. Create/repair CI for install, typecheck, lint, tests and static build
2. Configure Playwright browser setup if used
3. Cache dependencies safely
4. Audit `dist/` for server bundles, API routes and private fixtures
5. Document CI contract

### Explicit exclusions

- Do not deploy
- Do not add scheduled catalogue workflow

### Required verification

- Run the CI command sequence locally
...
- the TASK-013 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
