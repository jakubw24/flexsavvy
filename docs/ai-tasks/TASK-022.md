# TASK-022 — Implement timestamps with explicit offsets
...
| Suggested commit | `task-022: implement-timestamps-with-explicit-offsets` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-022 only**. Do not start another task.

### Objective

Normalize unambiguous timestamps to UTC and local derived fields.
...
1. Support ISO `Z` and explicit offsets
2. Support deterministic configured British formats
3. Normalize with Luxon
4. Derive Europe/London local date/time
5. Validate 30-minute alignment
6. Use start-inclusive/end-exclusive semantics
7. Return row errors

### Explicit exclusions

- Do not resolve ambiguous naive autumn times

### Required verification

- Test UTC, summer/winter offsets, invalid date, non-aligned time and chronology
...
- the TASK-022 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
