# TASK-047 — Add scheduled workflow and limited live verification
...
| Suggested commit | `task-047: add-scheduled-workflow-and-limited-live-verification` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-047 only**. Do not start another task.

### Objective

Automate safe refresh and verify live response shape once.
...
1. Create scheduled GitHub workflow for sync, validate and build
2. Never deploy when any job fails
3. Upload reports as artifacts
4. Run one separately labelled live check against required public endpoints only
5. Compare shape with fixtures
6. Document manual recovery
7. Audit no credentials/customer endpoints

### Explicit exclusions

- Do not auto-merge or auto-deploy
- Do not add other suppliers

### Required verification

- Run offline suite
- Run explicit live smoke command
...
- the TASK-047 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
