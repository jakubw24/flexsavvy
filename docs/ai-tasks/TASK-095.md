# TASK-095 — Implement SEO primitives, sitemap and robots

| Field | Value |
|---|---|
| Phase | 16 — Static guidance and SEO |
| Parent milestone | Milestone 18 |
| Risk | high |
| Network policy | `none` |
| Dependencies | TASK-012, TASK-094 |
| Suggested commit | `task-095: implement-seo-primitives-sitemap-and-robots` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-095 only**. Do not start another task.

### Objective

Create reusable static metadata and guide structure.
...
1. Implement title, description, canonical, social metadata and breadcrumbs
2. Generate sitemap/robots
3. Create guide layout
4. Support FAQ structured data only for visible matching FAQ
5. Never expose simulator state in metadata/URLs

### Explicit exclusions

- Do not write guide content or add tracking

### Required verification

- Test unique metadata, canonical routes, sitemap and FAQ consistency
...
- the TASK-095 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
