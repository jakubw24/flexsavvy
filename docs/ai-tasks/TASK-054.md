# TASK-054 — Implement plug-in windows and capacity
...
| Suggested commit | `task-054: implement-plug-in-windows-and-capacity` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-054 only**. Do not start another task.

### Objective

Map sessions to actual half-hour intervals.
...
1. Support same-day/overnight windows and selected days
2. Compute interval capacity as kW × 0.5h
3. Respect dataset boundaries
4. Handle UK DST using actual UTC intervals
5. Return total capacity and window warnings

### Explicit exclusions

- Do not allocate by price
- Do not model variable power

### Required verification

- Test normal overnight, spring, autumn, partial data and zero window
...
- the TASK-054 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
