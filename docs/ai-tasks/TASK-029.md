# TASK-029 — Implement Octopus JSON and CSV consumption adapters
...
| Suggested commit | `task-029: implement-octopus-json-and-csv-consumption-adapters` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-029 only**. Do not start another task.

### Objective

Normalize supported Octopus exports without credentials.
...
1. Accept JSON with consumption, interval_start, interval_end
2. Accept equivalent CSV
3. Validate ordering and 30-minute duration
4. Preserve row/index
5. Map to import kWh
6. Return structured field errors

### Explicit exclusions

- Do not call account API
- Do not request/store API keys

### Required verification

- Test JSON, CSV, reversed order, invalid duration, malformed records and DST
...
- the TASK-029 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
