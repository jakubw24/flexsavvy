# TASK-043 — Synchronize products and regional tariffs
...
| Suggested commit | `task-043: synchronize-products-and-regional-tariffs` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-043 only**. Do not start another task.

### Objective

Build typed intermediate data for active domestic electricity products.
...
1. Retrieve/filter active domestic products
2. Capture code, display name, validity and available eligibility metadata
3. Enumerate regional tariff codes
4. Preserve retrieval time
5. Return typed intermediate catalogue
6. Detect incomplete pagination
...
- Test active/inactive, missing region, pagination and incomplete response
...
- the TASK-043 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
