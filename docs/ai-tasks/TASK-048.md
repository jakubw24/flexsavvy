# TASK-048 — Implement appliance profiles and candidate generation
...
| Suggested commit | `task-048: implement-appliance-profiles-and-candidate-generation` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-048 only**. Do not start another task.

### Objective

Represent contiguous cycles and exhaustively generate valid starts.
...
1. Validate profile offsets/energy
2. Generate 30-minute starts on allowed days
3. Support earliest, latest and overnight windows
4. Map profile into canonical intervals
5. Preserve total cycle energy
6. Return impossible-window reasons

### Explicit exclusions

- Do not score or alter baseline

### Required verification

- Test one-slot, multi-slot, overnight, no candidate and DST windows
...
- the TASK-048 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
