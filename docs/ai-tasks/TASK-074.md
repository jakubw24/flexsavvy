# TASK-074 — Build quality and current tariff steps
...
| Risk | critical |
| Network policy | `none` |
| Dependencies | TASK-032, TASK-039, TASK-071, TASK-072, TASK-073 |
| Suggested commit | `task-074: build-quality-and-current-tariff-steps` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-074 only**. Do not start another task.

### Objective

Require understood data quality and a valid baseline tariff.
...
1. Render complete quality report
2. Block rejected data
3. Require acknowledgement where documented
4. Allow catalogue/manual/rate upload current tariff
5. Validate tariff coverage
6. Show replay/projection label
7. Keep tariff local

### Explicit exclusions

- Do not select candidates or calculate final results

### Required verification

- Test rejected data, warning acknowledgement, flat, TOU and interval current tariffs
...
- the TASK-074 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
