# TASK-041 — Implement aggregation, savings decomposition and precision audit
...
| Suggested commit | `task-041: implement-aggregation-savings-decomposition-and-precision-audit` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-041 only**. Do not start another task.

### Objective

Complete exact billing and transparent savings.
...
2. Return import, export, standing, degradation placeholder and net cost
3. Implement tariff-only, flexibility-only and combined formulas
4. Use integer minor units or controlled decimal accumulation
5. Round only display values
6. Add independent reference fixtures
7. Audit all pricing paths

### Explicit exclusions

- Do not implement optimisers
- Do not annualize incomplete data

### Required verification

- Test full-year penny stability, decomposition identities, negative prices, export and missing-rate failure
...
- the TASK-041 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
