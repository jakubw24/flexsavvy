# TASK-067 — Implement regional fetch and public cache
...
| Suggested commit | `task-067: implement-regional-fetch-and-public-cache` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-067 only**. Do not start another task.

### Objective

Fetch only required public interval ranges and cache no private state.
...
1. Allow manual region
2. Fetch only simulation dates
3. Use narrow fetch wrapper
4. Cache public region/date responses with expiry
5. Never combine cache with consumption or settings
6. Support clear cache

### Explicit exclusions

- Do not request postcode or persist consumption

### Required verification

- Test range, cache hit/expiry/clear and forbidden fields
...
- the TASK-067 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
