# TASK-017 — Implement appliance, EV, battery and carbon schemas
...
| Suggested commit | `task-017: implement-appliance-ev-battery-and-carbon-schemas` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-017 only**. Do not start another task.

### Objective

Validate all flexibility inputs and cross-field rules.
...
1. Implement appliance profiles, windows, frequency and constraints
2. Implement EV modes, energy methods, power, efficiency, windows and current profile
3. Implement battery capacity, reserve, SOC, powers, efficiencies, flags, degradation and investment
4. Implement carbon region and weights
5. Add cross-field refinements

### Explicit exclusions

- Do not implement optimisers or forms

### Required verification

- Test valid/invalid cross-field combinations, efficiencies, overnight windows and reserves
...
- the TASK-017 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
