# TASK-070 — Integrate graceful failure and live-check carbon

| Field | Value |
|---|---|
| Phase | 11 — Carbon data |
| Parent milestone | Milestone 13 |
| Risk | critical |
| Network policy | `limited-live` |
| Dependencies | TASK-066, TASK-067, TASK-068, TASK-069 |
| Suggested commit | `task-070: integrate-graceful-failure-and-live-check-carbon` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-070 only**. Do not start another task.

### Objective

Complete optional integration and verify live response shape once.
...
1. Integrate into appliance/EV/battery only when adequate coverage
2. Continue cost simulation on failure
3. Show coverage/warnings
4. Run one public endpoint live shape check
5. Update fixtures only from verified fields
6. Audit payload/cache privacy

### Explicit exclusions

- Do not send consumption
- Do not block costs

### Required verification

- Run offline suite and explicit live smoke
...
The policy for this task is **`limited-live`**.
...
- the TASK-070 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
