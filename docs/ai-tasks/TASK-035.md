# TASK-035 — Add overnight, weekday and DST-safe TOU periods
...
| Suggested commit | `task-035: add-overnight-weekday-and-dst-safe-tou-periods` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-035 only**. Do not start another task.

### Objective

Support complete UK local tariff schedules.
...
1. Support periods crossing midnight
2. Define owning local day consistently
3. Support weekday/weekend sets
4. Handle spring and both autumn occurrences using actual interval local representation
5. Never assume one repeated local time

### Explicit exclusions

- Do not implement overlap priority

### Required verification

- Test overnight, Friday/Saturday, spring gap and both autumn intervals
...
- the TASK-035 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
