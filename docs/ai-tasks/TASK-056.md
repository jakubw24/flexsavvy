# TASK-056 — Implement future and shift-existing modes
...
| Suggested commit | `task-056: implement-future-and-shift-existing-modes` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-056 only**. Do not start another task.

### Objective

Apply optimized EV profiles with explicit modelling boundaries.
...
1. Add optimized profile for future EV
2. For shift mode construct current profile, subtract safely and add optimized
3. Never make baseline negative
4. Record modelled-not-detected warning
5. Keep original immutable
6. Return before/after intervals

### Explicit exclusions

- Do not disaggregate EV from aggregate data

### Required verification

- Test future addition, exact shift, insufficient baseline, immutability and overnight current profile
...
- the TASK-056 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
