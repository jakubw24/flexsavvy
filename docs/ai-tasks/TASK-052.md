# TASK-052 — Verify appliance optimiser with brute force and invariants
...
| Risk | critical |
| Network policy | `none` |
| Dependencies | TASK-048, TASK-049, TASK-050, TASK-051 |
| Suggested commit | `task-052: verify-appliance-optimiser-with-brute-force-and-invariants` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-052 only**. Do not start another task.

### Objective

Prove optimality and energy preservation.
...
1. Implement independent test-only brute-force solver
2. Compare small cases
3. Add properties for energy, windows, non-negative baseline and optimum
4. Audit DST, negative prices and ties
5. Define worker-serializable request/result shape

### Explicit exclusions

- Do not move execution to worker yet
- Do not share production scoring code with reference solver

### Required verification

- Run exhaustive small cases and fixed-seed properties
...
- the TASK-052 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
