# TASK-034 — Implement basic TOU resolution
...
| Suggested commit | `task-034: implement-basic-tou-resolution` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-034 only**. Do not start another task.

### Objective

Resolve local wall-time periods for simple same-day schedules.
...
1. Convert UTC start to Europe/London
2. Match local time and weekday
3. Use explicit default outside periods
4. Use start-inclusive/end-exclusive
5. Return matched period ID

### Explicit exclusions

- Do not support midnight crossing or overlap yet

### Required verification

- Test daytime, boundaries, default and weekday
...
- the TASK-034 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
