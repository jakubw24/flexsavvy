# TASK-027 — Implement n3rgy CSV adapter
...
| Suggested commit | `task-027: implement-n3rgy-csv-adapter` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-027 only**. Do not start another task.

### Objective

Normalize supported n3rgy exports.
...
1. Detect timestamp, value and `device_ID`
2. Preserve device ID without logging
3. Support verified fixture variants
4. Map values to import kWh
5. Return format warnings
6. Never combine devices automatically

### Explicit exclusions

- Do not call n3rgy services
- Do not choose device

### Required verification

- Fixtures for one/multiple devices, absent ID, malformed values and DST
...
- the TASK-027 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
