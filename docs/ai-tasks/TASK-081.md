# TASK-081 — Implement tariff table and savings waterfall
...
| Suggested commit | `task-081: implement-tariff-table-and-savings-waterfall` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-081 only**. Do not start another task.

### Objective

Explain how final cost is reached.
...
1. Build accessible tariff comparison table
2. Build waterfall from current cost through savings/cost additions
3. Include battery degradation
4. Label ineligible modelled tariffs
5. Provide text equivalent

### Explicit exclusions

- Do not use chart-only meaning or commercial ranking

### Required verification

- Test negative saving, degradation, no flexibility and ineligible tariff
...
- the TASK-081 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
