# TASK-023 — Implement naive timestamp and DST handling

| Field | Value |
|---|---|
| Phase | 4 — Generic import |
| Parent milestone | Milestone 4 |
| Risk | critical |
| Network policy | `none` |
| Dependencies | TASK-022 |
| Suggested commit | `task-023: implement-naive-timestamp-and-dst-handling` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-023 only**. Do not start another task.

### Objective

Handle UK local timestamps without silently corrupting DST days.
...
1. Support configured naive formats in Europe/London
2. Represent spring 46-interval day
3. Detect repeated autumn times
4. Require offset column, order rule or user choice to resolve ambiguity
5. Never fabricate intervals
6. Return ambiguity objects for UI

### Explicit exclusions

- Do not silently select an autumn occurrence
- Do not interpolate

### Required verification

- Test spring gap, both autumn occurrences, repeated time, missing time and normal day
...
- the TASK-023 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
