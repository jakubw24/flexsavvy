# TASK-021 — Implement numeric parsing and unit conversion
...
| Suggested commit | `task-021: implement-numeric-parsing-and-unit-conversion` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-021 only**. Do not start another task.

### Objective

Normalize supported numeric values to canonical kWh.
...
1. Parse decimal points and supported decimal commas without delimiter corruption
2. Trim whitespace
3. Support kWh and Wh
4. Convert Wh to kWh
5. Reject NaN, infinity and negative import/export
6. Return row-specific errors
7. Require confirmation when units are ambiguous
...
- Test decimal variants, Wh conversion, blank, negative, NaN, whitespace and large values
...
- the TASK-021 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
