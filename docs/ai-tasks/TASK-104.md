# TASK-104 — Harden worker, cancellation and privacy tests
...
| Suggested commit | `task-104: harden-worker-cancellation-and-privacy-tests` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-104 only**. Do not start another task.

### Objective

Verify concurrency and fail-closed privacy.
...
1. Test worker serialization and monotonic progress
2. Test cancellation before/mid/near completion
3. Test stale results after input change
4. Test fetch deny-list/storage
5. Test export has no network

### Explicit exclusions

- Do not use timing-fragile sleeps when controllable

### Required verification

- Run worker harness and Playwright interception
...
- the TASK-104 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
