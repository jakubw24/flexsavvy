# TASK-062 — Add terminal SOC and rolling 48-hour horizons
...
| Suggested commit | `task-062: add-terminal-soc-and-rolling-48-hour-horizons` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-062 only**. Do not start another task.

### Objective

Prevent terminal dumping and support long data.
...
1. Require final SOC >= starting SOC on final horizon
2. Optimize 48h and commit first 24h
3. Carry SOC
4. Handle short final horizon
5. Commit every interval exactly once
6. Return horizon diagnostics

### Explicit exclusions

- Do not add policy flags or parallelism

### Required verification

- Test 24h, 48h, 72h, short final horizon and terminal high-price temptation
...
- the TASK-062 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
