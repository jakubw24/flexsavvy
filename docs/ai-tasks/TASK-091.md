# TASK-091 — Implement central fetch wrapper and deny-list
...
| Suggested commit | `task-091: implement-central-fetch-wrapper-and-deny-list` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-091 only**. Do not start another task.

### Objective

Fail closed on unknown or sensitive requests.
...
1. Route application fetches through typed wrapper
2. Allow configured static tariff/carbon destinations only
3. Inspect query/headers/body for forbidden keys/values in dev/tests
4. Reject unknown destinations
5. Support timeout/cancel/safe errors
6. Never echo sensitive values

### Explicit exclusions

- Do not route build scripts through browser wrapper

### Required verification

- Test allowed static/carbon, unknown host, nested forbidden, filename and device ID
...
- the TASK-091 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
