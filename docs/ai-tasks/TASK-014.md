# TASK-014 — Implement shared primitives, warnings and units
...
| Suggested commit | `task-014: implement-shared-primitives-warnings-and-units` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-014 only**. Do not start another task.

### Objective

Create reusable validated primitives.
...
1. Implement named/branded UTC timestamp, local date/time, kWh, pence-rate and percentage types where useful
2. Implement warning severity/codes/messages
3. Implement confidence levels
4. Implement finite-number helpers
5. Use Zod and inferred types

### Explicit exclusions

- Do not implement entity schemas or UI

### Required verification

- Test invalid times, percentages, NaN/infinity and warning serialization
...
- the TASK-014 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
