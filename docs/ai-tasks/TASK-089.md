# TASK-089 — Audit export privacy and consistency
...
| Suggested commit | `task-089: audit-export-privacy-and-consistency` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-089 only**. Do not start another task.

### Objective

Verify all formats reconcile and stay local.
...
1. Compare JSON, CSV, print and PDF with result state
2. Verify rounding
3. Verify no filename/device ID/hidden raw data
4. Intercept network during export
5. Test delete after export
6. Repair export-only defects

### Explicit exclusions

- Do not change core engine formulas without proven defect

### Required verification

- Run full export unit and Playwright scenarios
...
- the TASK-089 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
