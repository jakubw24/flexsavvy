# TASK-101 — Harden billing, tariff and DST tests
...
| Suggested commit | `task-101: harden-billing-tariff-and-dst-tests` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-101 only**. Do not start another task.

### Objective

Prove exact billing over all canonical cases.
...
1. Add manual reference tests for all tariff/billing components
2. Add spring/autumn rate tests
3. Add missing/duplicate failures
4. Add full-year penny stability
5. Add property that each billed interval has exactly one import rate and valid export handling

### Explicit exclusions

- Do not adjust expected values to match faulty output
...
- the TASK-101 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
