# TASK-042 — Implement isolated Octopus public API client with fixtures
...
| Dependencies | TASK-016, TASK-041 |
| Suggested commit | `task-042: implement-isolated-octopus-public-api-client-with-fixtures` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-042 only**. Do not start another task.

### Objective

Create a typed, mockable public API boundary.
...
1. Create narrow HTTP interface and typed raw models
2. Implement pagination, timeout and bounded retry
3. Store sanitized fixtures
4. Mock all automated tests
5. Do not guess undocumented fields
6. Never access customer endpoints

### Explicit exclusions

- Do not write catalogue files
- Do not make live calls in tests

### Required verification

- Test pagination, timeout, non-200, malformed JSON and fixture parsing
...
- the TASK-042 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
