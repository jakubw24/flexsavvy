...
| Suggested commit | `task-007: create-privacy-design` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-007 only**. Do not start another task.

### Objective

Define browser-only processing and permitted network boundaries.
...
1. Create/update `docs/PRIVACY_DESIGN.md`
2. Classify consumption, filenames, device IDs, tariffs, schedules and results
3. List permitted outbound requests and forbidden fields
4. Define memory lifecycle, deletion, storage and analytics restrictions
5. Define public-data cache boundary
6. Define future connected-data work as out of scope
7. Define browser network test strategy
8. Add threat model for telemetry, logs, scripts and exports
...
- Cross-check privacy statements against product journey and schema
...
- the TASK-007 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
