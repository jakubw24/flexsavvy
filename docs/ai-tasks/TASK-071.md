# TASK-071 — Implement typed wizard state and reducer
...
| Risk | critical |
| Network policy | `none` |
| Dependencies | TASK-018, TASK-032, TASK-039, TASK-052, TASK-057, TASK-065, TASK-070 |
| Suggested commit | `task-071: implement-typed-wizard-state-and-reducer` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-071 only**. Do not start another task.

### Objective

Create a single deterministic state model for the 11-step flow.
...
1. Define state for source, file, parsed data, mapping, quality, tariffs, appliances, EV, battery, run and result
2. Implement typed actions/reducer
3. Preserve parsed data when editing later steps
4. Invalidate only dependent results
5. Keep sensitive state in memory
6. Test every transition

### Explicit exclusions

- Do not build screens or persist state

### Required verification

- Test forward/back edits, dependent invalidation, reset and cancel state
...
- the TASK-071 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
