# TASK-073 — Build data source, upload and mapping steps
...
| Risk | critical |
| Network policy | `none` |
| Dependencies | TASK-019, TASK-024, TASK-025, TASK-026, TASK-027, TASK-029, TASK-071, TASK-072 |
| Suggested commit | `task-073: build-data-source-upload-and-mapping-steps` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-073 only**. Do not start another task.

### Objective

Integrate all import routes into the wizard.
...
1. Offer CSV, Octopus JSON/CSV and sample data
2. Run adapter detection
3. Show generic mapping when needed
4. Preview first five rows
5. Resolve units/timestamp ambiguity explicitly
6. Support replace/cancel
7. Store normalized data only after success

### Explicit exclusions

- Do not show quality yet
- Do not request credentials

### Required verification

- Playwright flows for generic, n3rgy, Octopus and sample input
...
- the TASK-073 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
