# TASK-086 — Implement scenario JSON and interval CSV export
...
| Risk | critical |
| Network policy | `none` |
| Dependencies | TASK-018, TASK-085 |
| Suggested commit | `task-086: implement-scenario-json-and-interval-csv-export` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-086 only**. Do not start another task.

### Objective

Provide local machine-readable outputs.
...
1. Export versioned assumptions JSON
2. Export interval results CSV
3. Exclude raw consumption from scenario JSON unless explicitly selected
4. Generate safe local filenames not based on upload name
5. Validate JSON
6. Include generation time/projection

### Explicit exclusions

- Do not upload or implement PDF

### Required verification

- Test JSON round-trip, CSV escaping/columns, large data and excluded fields
...
- the TASK-086 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
