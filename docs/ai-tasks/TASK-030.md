# TASK-030 — Implement expected, missing and duplicate intervals
...
| Risk | critical |
| Network policy | `none` |
| Dependencies | TASK-025, TASK-028, TASK-029 |
| Suggested commit | `task-030: implement-expected-missing-and-duplicate-intervals` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-030 only**. Do not start another task.

### Objective

Calculate temporal coverage correctly across UK DST.
...
1. Generate expected UTC half-hour starts
2. Identify missing and duplicates
3. Report complete local days without assuming 48
4. Treat spring 46 and autumn 50 as valid
5. Separate same-device duplicate from multi-device overlap
6. Return sorted deterministic results
...
- Test normal, spring, autumn, missing, duplicate and unordered
...
- the TASK-030 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
