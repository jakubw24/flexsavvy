# TASK-110 — Run release-readiness engineering audit
...
| Suggested commit | `task-110: run-release-readiness-engineering-audit` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-110 only**. Do not start another task.

### Objective

Verify architecture, privacy, calculations, tests and deployment from a clean install.
...
1. Review repo/build
2. Verify no backend/account/upload/sensitive persistence
3. Verify independent critical tests
4. Verify task/progress evidence
5. Run clean install full sequence
6. List blockers by severity
7. Repair only clearly scoped defects

### Explicit exclusions

- Do not declare ready with critical/high blockers
- Do not deploy

### Required verification

- Run clean install, typecheck, lint, unit, build and E2E
...
- the TASK-110 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
