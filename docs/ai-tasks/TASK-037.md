# TASK-037 — Implement interval pricing and missing-price handling
...
| Suggested commit | `task-037: implement-interval-pricing-and-missing-price-handling` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-037 only**. Do not start another task.

### Objective

Resolve dynamic rates by exact UTC interval.
...
1. Index rates by UTC start
2. Preserve negative prices
3. Detect duplicate prices
4. Return all missing required intervals
5. Never fill missing rates
6. Keep source metadata
...
- Test exact, negative, missing, duplicate, unsorted and DST data
...
- the TASK-037 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
