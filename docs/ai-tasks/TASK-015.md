# TASK-015 — Implement consumption and quality schemas
...
| Suggested commit | `task-015: implement-consumption-and-quality-schemas` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-015 only**. Do not start another task.

### Objective

Validate canonical intervals and data-quality outputs.
...
1. Implement `ConsumptionInterval` with UTC boundaries, local fields, 30-minute duration, import, optional export, device ID and source row
2. Require finite non-negative kWh
3. Implement missing, duplicate, device summary, quality and rejection schemas
...
- Test valid/invalid intervals, negative energy, malformed timestamps, nullable export and quality objects
...
- the TASK-015 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
