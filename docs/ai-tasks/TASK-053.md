# TASK-053 — Implement EV validation and energy requirement
...
| Suggested commit | `task-053: implement-ev-validation-and-energy-requirement` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-053 only**. Do not start another task.

### Objective

Calculate required grid energy from supported inputs.
...
1. Support weekly miles / miles-per-kWh
2. Support energy/session × sessions/week
3. Apply charging efficiency
4. Reject contradictory/incomplete methods
5. Calculate session requirement deterministically
6. Return units and assumptions

### Explicit exclusions

- Do not schedule
- Do not annualize low-confidence data

### Required verification

- Test both methods, efficiency, zero and invalid combinations
...
- the TASK-053 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
