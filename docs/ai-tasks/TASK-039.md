# TASK-039 — Build manual tariff forms and rate CSV import
...
| Suggested commit | `task-039: build-manual-tariff-forms-and-rate-csv-import` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-039 only**. Do not start another task.

### Objective

Allow local definition of all supported tariffs.
...
1. Build accessible flat and TOU forms
2. Capture standing charge, validity, region and eligibility
3. Import interval-rate CSV using safe parser
4. Validate before adding
5. Show sample rates/errors
6. Keep entries local

### Explicit exclusions

- Do not build catalogue selector
- Do not persist by default

### Required verification

- Component/Playwright tests for flat, invalid, overnight and dynamic upload
...
- the TASK-039 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
