# TASK-046 — Implement atomic catalogue writing and validation
...
| Suggested commit | `task-046: implement-atomic-catalogue-writing-and-validation` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-046 only**. Do not start another task.

### Objective

Prevent failed synchronization from corrupting the last valid catalogue.
...
1. Write to temporary generation directory
2. Validate counts, references, schemas and coverage
3. Promote only after success
4. Retain/restore prior valid generation on failure
5. Produce machine-readable report
6. Leave no partial files

### Explicit exclusions

- Do not add scheduled workflow
- Do not make live calls

### Required verification

- Test success, schema failure, missing rate, interrupted write and rollback
...
- the TASK-046 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
