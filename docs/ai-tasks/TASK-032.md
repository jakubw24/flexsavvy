# TASK-032 — Implement acceptance, confidence and quality report

| Field | Value |
|---|---|
| Phase | 5 — Adapters and quality |
| Parent milestone | Milestone 6 |
| Risk | critical |
| Network policy | `none` |
| Dependencies | TASK-030, TASK-031 |
| Suggested commit | `task-032: implement-acceptance-confidence-and-quality-report` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-032 only**. Do not start another task.

### Objective

Turn quality metrics into transparent reject/warn/confidence outcomes.
...
1. Reject <7 complete days, <80% coverage, unresolved negative values and unorderable timestamps
2. Classify high/medium/low using docs
3. Gate prominent annual estimates
4. Build accessible quality report with coverage, dates, missing, duplicate, totals, devices and ambiguity
5. Audit all adapters

### Explicit exclusions

- Do not interpolate
- Do not implement tariffs

### Required verification

- Test threshold boundaries and full fixture UI flows
...
- the TASK-032 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
