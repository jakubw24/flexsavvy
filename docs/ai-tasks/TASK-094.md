# TASK-094 — Run browser network interception and privacy audit
...
| Risk | critical |
| Network policy | `none` |
| Dependencies | TASK-090, TASK-091, TASK-092, TASK-093 |
| Suggested commit | `task-094: run-browser-network-interception-and-privacy-audit` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-094 only**. Do not start another task.

### Objective

Prove sentinel sensitive values never leave the browser.
...
1. Intercept every request across import, mapping, quality, tariffs, simulation, results and export
2. Use distinctive sentinels
3. Fail on URL/header/body leakage
4. Inspect console/storage
5. Verify public requests contain allowed fields only
6. Update privacy evidence

### Explicit exclusions

- Do not weaken assertions or add analytics

### Required verification

- Run complete Playwright privacy suite
...
- the TASK-094 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
