# TASK-076 — Build appliance configuration step
...
| Risk | high |
| Network policy | `none` |
| Dependencies | TASK-051, TASK-071, TASK-072 |
| Suggested commit | `task-076: build-appliance-configuration-step` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-076 only**. Do not start another task.

### Objective

Configure multiple explicit appliance models.
...
1. Allow zero/multiple appliances
2. Offer editable templates/custom profiles
3. Capture cycles, current start, windows, days and power limit
4. Validate overnight windows
5. Show baseline modelling warning
6. Support edit/duplicate/remove

### Explicit exclusions

- Do not run optimiser or auto-detect appliances

### Required verification

- Test templates, custom, invalid window, multiple and removal
...
- the TASK-076 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
