# TASK-010 — Configure strict TypeScript and repository scripts
...
| Suggested commit | `task-010: configure-strict-typescript-and-repository-scripts` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-010 only**. Do not start another task.

### Objective

Make quality commands explicit and reproducible.
...
2. Add/verify dev, build, preview, typecheck, test, test:e2e and lint scripts
3. Document Node and package-manager versions
4. Avoid duplicate tooling
5. Repair scoped type errors instead of weakening checks
...
- Execute every configured non-browser command
- Check changed code introduces no `any`
...
- the TASK-010 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
