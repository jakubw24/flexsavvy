# TASK-019 — Implement browser file selection, limits and safe reading
...
| Suggested commit | `task-019: implement-browser-file-selection-limits-and-safe-reading` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-019 only**. Do not start another task.

### Objective

Create the browser-only file ingestion boundary.
...
1. Accept CSV and JSON
2. Reject files above 25 MB
3. Read only in browser memory
4. Do not log filename/content
5. Support cancel/reset
6. Define typed raw-file and read-result objects

### Explicit exclusions

- Do not parse file contents
- Do not persist files

### Required verification

- Test accepted type, rejected size, cancelled read and zero network requests
...
- the TASK-019 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
