# TASK-020 — Implement CSV parsing and delimiter detection
...
| Suggested commit | `task-020: implement-csv-parsing-and-delimiter-detection` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-020 only**. Do not start another task.

### Objective

Parse raw CSV into rows with structured errors.
...
1. Use Papa Parse
2. Detect comma, semicolon and tab
3. Handle quoted fields and BOM
4. Preserve source row numbers
5. Return headers, rows, empty-row info and parse errors
6. Never throw unhandled malformed-row errors

### Explicit exclusions

- Do not interpret timestamps or energy

### Required verification

- Fixtures for comma, semicolon, tab, BOM, quotes, malformed row and empty file
...
- the TASK-020 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
