# TASK-059 — Implement actions and state transitions
...
| Suggested commit | `task-059: implement-actions-and-state-transitions` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-059 only**. Do not start another task.

### Objective

Generate valid charge/idle/discharge transitions.
...
1. Generate signed actions for 30-minute intervals
2. Respect max powers
3. Use separate charge/discharge efficiency
4. Prevent simultaneous charge/discharge by construction
5. Reject SOC outside bounds
6. Document grid-side versus battery-side energy

### Explicit exclusions

- Do not calculate tariff cost or choose actions

### Required verification

- Test charge, discharge, idle, loss, bounds and discretisation
...
- the TASK-059 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
