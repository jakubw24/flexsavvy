# TASK-025 — Implement generic consumption adapter
...
| Suggested commit | `task-025: implement-generic-consumption-adapter` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-025 only**. Do not start another task.

### Objective

Convert parsed rows and confirmed mapping to canonical intervals.
...
1. Apply numeric, unit and timestamp parsing
2. Create canonical intervals with source row
3. Preserve export/device ID
4. Return accepted/rejected rows, warnings and unresolved ambiguities
5. Sort only after parsing while retaining original-order evidence
...
- Integration fixtures for ISO CSV, British CSV, Wh, malformed rows and DST ambiguity
...
- the TASK-025 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
