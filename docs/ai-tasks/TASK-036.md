# TASK-036 — Implement TOU overlap priority, eligibility and validity

| Field | Value |
|---|---|
| Phase | 6 — Tariffs and billing |
| Parent milestone | Milestone 7 |
| Risk | critical |
| Network policy | `none` |
| Dependencies | TASK-035 |
| Suggested commit | `task-036: implement-tou-overlap-priority-eligibility-and-validity` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-036 only**. Do not start another task.

### Objective

Make complex tariff definitions deterministic and transparent.
...
1. Resolve overlap by explicit numeric priority
2. Reject equal-priority ambiguity
3. Validate complete rate coverage
4. Evaluate region/equipment eligibility separately
5. Preserve eligibility notes
6. Apply effective dates

### Explicit exclusions

- Do not hide ineligible tariffs

### Required verification

- Test priority, tie rejection, missing coverage, region mismatch and equipment restrictions
...
- the TASK-036 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
