# TASK-024 — Implement column detection and mapping model
...
| Suggested commit | `task-024: implement-column-detection-and-mapping-model` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-024 only**. Do not start another task.

### Objective

Suggest mappings with confidence while preserving user control.
...
1. Score headers and samples for timestamp, import, export, device ID and unit
2. Return confidence and reasons
3. Allow overrides
4. Prevent incompatible reuse of one column
5. Require timestamp and import
6. Allow absent export/device
7. Represent unit and timestamp format explicitly

### Explicit exclusions

- Do not build full UI
- Do not auto-accept low confidence

### Required verification

- Test common/misleading headers, no match, duplicate role and unit ambiguity
...
- the TASK-024 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
