# TASK-078 — Implement simulation coordinator and worker orchestration
...
| Risk | critical |
| Network policy | `none` |
| Dependencies | TASK-041, TASK-052, TASK-057, TASK-065, TASK-070, TASK-071, TASK-072, TASK-073, TASK-074, TASK-075, TASK-076, TASK-077 |
| Suggested commit | `task-078: implement-simulation-coordinator-and-worker-orchestration` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-078 only**. Do not start another task.

### Objective

Run baseline, candidates and selected flexibility consistently.
...
1. Define exact engine order
2. Send serializable inputs to local worker
3. Report stage/percent
4. Support cancel and ignore stale messages
5. Never mutate baseline between candidates
6. Collect warnings/assumptions
7. Clear results when dependencies change

### Explicit exclusions

- Do not build final dashboard or persist raw data

### Required verification

- Test success, cancel each stage, worker error, carbon failure and multiple candidates
...
- the TASK-078 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
