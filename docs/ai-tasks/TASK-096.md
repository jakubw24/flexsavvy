# TASK-096 — Create smart-meter download and n3rgy guides
...
| Suggested commit | `task-096: create-smart-meter-download-and-n3rgy-guides` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-096 only**. Do not start another task.

### Objective

Publish accurate import-focused guidance.
...
1. Create general half-hourly data download guide
2. Create n3rgy CSV import guide
3. Explain required columns, data length, privacy and device handling
4. Use only owned/current screenshots
5. Link sample and simulator
6. Avoid unsupported supplier claims

### Explicit exclusions

- Do not fabricate steps or copy third-party docs

### Required verification

- Run link, metadata and build checks
...
- the TASK-096 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
