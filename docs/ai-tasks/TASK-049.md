# TASK-049 — Implement constraints and baseline subtraction
...
| Suggested commit | `task-049: implement-constraints-and-baseline-subtraction` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-049 only**. Do not start another task.

### Objective

Apply power limits and safely remove a modelled current cycle.
...
1. Reject candidates exceeding optional household power limit
2. Construct current profile
3. Subtract by interval
4. Never produce negative consumption
5. Return insufficient-baseline evidence
6. Keep input immutable

### Explicit exclusions

- Do not select optimum
- Do not detect appliances from aggregate data

### Required verification

- Test exact/insufficient subtraction, power boundary and immutability
...
- the TASK-049 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
