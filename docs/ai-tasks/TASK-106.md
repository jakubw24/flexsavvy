# TASK-106 — Audit production build and code splitting
...
| Suggested commit | `task-106: audit-production-build-and-code-splitting` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-106 only**. Do not start another task.

### Objective

Ensure efficient static output with no server assumptions.
...
1. Audit bundles
2. Lazy-load charts/PDF/simulator
3. Verify all routes static
4. Verify workers resolve
5. Exclude private fixtures/tests
6. Control production source maps
7. Document artifact contents

### Explicit exclusions

- Do not deploy or remove development maps

### Required verification

- Run build/bundle/route smoke checks
...
- the TASK-106 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
