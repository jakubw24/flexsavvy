# TASK-105 — Complete E2E scenarios and mutation-oriented audit

| Field | Value |
|---|---|
| Phase | 17 — Validation suite |
| Parent milestone | Milestone 19 |
| Risk | critical |
| Network policy | `none` |
| Dependencies | TASK-100, TASK-101, TASK-102, TASK-103, TASK-104 |
| Suggested commit | `task-105: complete-e2e-scenarios-and-mutation-oriented-audit` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-105 only**. Do not start another task.

### Objective

Ensure plausible critical defects are caught.
...
1. Add full sample journey with all features
2. Add rejected-data, missing dynamic price and delete journeys
3. Consider mutations: wrong DST, omitted standing, reversed saving, ignored efficiency, removed terminal SOC
4. Add assertions catching each
5. Document residual risk

### Explicit exclusions

- Do not add mutation dependency unless justified
- Avoid brittle pixel-only tests
...
- the TASK-105 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
