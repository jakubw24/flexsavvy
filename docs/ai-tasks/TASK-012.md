# TASK-012 — Create static pages, layout and navigation
...
| Suggested commit | `task-012: create-static-pages-layout-and-navigation` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-012 only**. Do not start another task.

### Objective

Build the accessible static shell.
...
1. Create/verify `/`, `/simulator`, `/methodology`, `/privacy`, `/terms`, `/sample-report`
2. Create semantic shared layout and keyboard-accessible navigation
3. Keep simulator free of third-party scripts
4. Use placeholders only for later-owned content
5. Ensure direct refresh works

### Explicit exclusions

- Do not build wizard or charts

### Required verification

- Add route and keyboard navigation Playwright tests
...
- the TASK-012 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
