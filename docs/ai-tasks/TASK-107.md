# TASK-107 — Verify routes, asset paths and cache policy
...
| Suggested commit | `task-107: verify-routes-asset-paths-and-cache-policy` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-107 only**. Do not start another task.

### Objective

Make deployment reliable at root or configured base path.
...
1. Verify direct refresh and nested routes
2. Verify asset/worker/PDF base paths
3. Define immutable hashed assets, short-lived public JSON and no-cache HTML
4. Add 404
5. Document subdomain vs subdirectory

### Explicit exclusions

- Do not choose domain or deploy

### Required verification

- Run preview at root and non-root base where supported
...
- the TASK-107 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
