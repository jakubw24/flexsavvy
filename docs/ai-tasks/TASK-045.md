# TASK-045 — Normalize catalogue and generate browser index
...
| Suggested commit | `task-045: normalize-catalogue-and-generate-browser-index` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-045 only**. Do not start another task.

### Objective

Convert API data into validated internal tariff definitions.
...
1. Normalize product-region combinations to `TariffDefinition`
2. Map pricing, standing, validity, source and eligibility
3. Generate compact index and separate rate files
4. Validate with Zod
5. Order output deterministically
6. State catalogue is not whole market

### Explicit exclusions

- Do not implement atomic promotion or UI

### Required verification

- Test schema validity, stable ordering and index references
...
- the TASK-045 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
