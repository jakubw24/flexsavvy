# TASK-082 — Implement monthly and load-profile visualisations
...
| Suggested commit | `task-082: implement-monthly-and-load-profile-visualisations` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-082 only**. Do not start another task.

### Objective

Show temporal cost and consumption changes honestly.
...
1. Create monthly cost comparison
2. Create average half-hour profile before/after
3. Create price/consumption overlay
4. Mark missing data/incomplete months
5. Lazy-load charts
6. Provide tables/text summaries

### Explicit exclusions

- Do not average away missing intervals or put raw data in URLs

### Required verification

- Test incomplete month, DST, missing price and responsive rendering
...
- the TASK-082 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
