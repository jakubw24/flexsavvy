# TASK-075 — Build candidate tariff selection
...
| Risk | critical |
| Network policy | `none` |
| Dependencies | TASK-039, TASK-047, TASK-071, TASK-072, TASK-074 |
| Suggested commit | `task-075: build-candidate-tariff-selection` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-075 only**. Do not start another task.

### Objective

Select transparent valid comparison tariffs.
...
1. Load static catalogue index
2. Filter by region/validity while retaining eligibility
3. Allow multiple candidates and manual entries
4. Show rate type, standing, validity, source and eligibility
5. Reject incomplete rates
6. State catalogue coverage limitation

### Explicit exclusions

- Do not rank by affiliate value or run final simulation

### Required verification

- Test load failure, ineligible, manual and missing-rate candidates
...
- the TASK-075 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
