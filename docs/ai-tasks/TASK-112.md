# TASK-112 — Implement privacy-safe funnel instrumentation interface
...
| Risk | critical |
| Network policy | `none` |
| Dependencies | TASK-092, TASK-111 |
| Suggested commit | `task-112: implement-privacy-safe-funnel-instrumentation-interface` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-112 only**. Do not start another task.

### Objective

Measure usability without collecting simulator values.
...
1. Define event names for CTA, start, coarse import outcome, completion, report and pro interest
2. Allow coarse enumerated failures only
3. Build provider-neutral interface disabled by default
4. Add consent/config gate
5. Reject numeric household values, tariffs, filename, ID and schedules
6. Use local test sink

### Explicit exclusions

- Do not integrate vendor, fingerprint or send by default

### Required verification

- Test allowed/forbidden events and disabled full journey
...
- the TASK-112 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
