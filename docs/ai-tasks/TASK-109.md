# TASK-109 — Implement rollback and smoke procedures
...
| Suggested commit | `task-109: implement-rollback-and-smoke-procedures` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-109 only**. Do not start another task.

### Objective

Make releases recoverable and testable.
...
1. Document versioned releases and rollback
2. Create smoke checklist for pages, import, tariffs, worker, charts, export, delete, mobile and network
3. Create base-URL Playwright smoke option
4. Require HTTPS
5. Record release metadata/checksums
6. Keep rollback manual

### Explicit exclusions

- Do not access production without explicit URL/authorization or alter DNS

### Required verification

- Test smoke suite against local preview
...
- the TASK-109 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
