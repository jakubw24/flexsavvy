# TASK-064 — Implement battery metrics and results
...
| Suggested commit | `task-064: implement-battery-metrics-and-results` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-064 only**. Do not start another task.

### Objective

Transform optimized path into interval and summary outputs.
...
1. Return SOC, charge, discharge, import/export per interval
2. Calculate gross saving, degradation and net
3. Calculate throughput/equivalent cycles
4. Calculate peak import reduction
5. Return assumptions/warnings
6. Gate payback

### Explicit exclusions

- Do not build UI
- Do not claim exact lifetime return

### Required verification

- Test cycles, unused battery, negative saving, peak reduction and low-confidence payback
...
- the TASK-064 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
