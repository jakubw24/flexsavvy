# TASK-080 — Implement summary, decomposition and confidence
...
| Risk | critical |
| Network policy | `none` |
| Dependencies | TASK-041, TASK-078, TASK-079 |
| Suggested commit | `task-080: implement-summary-decomposition-and-confidence` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-080 only**. Do not start another task.

### Objective

Lead with reconciled financial outcomes and confidence.
...
1. Show period, current cost, lowest unchanged cost, lowest optimized cost and combined saving
2. Show tariff-only/flexibility-only/combined
3. Show projection type and confidence reasons
4. Gate annual figures
5. Use result fields without component recalculation

### Explicit exclusions

- Do not add charts
- Do not hide negative savings

### Required verification

- Test reconciliation, low confidence, replay and projection
...
- the TASK-080 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
