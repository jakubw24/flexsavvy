# TASK-093 — Implement security headers and CSP guidance
...
| Suggested commit | `task-093: implement-security-headers-and-csp-guidance` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-093 only**. Do not start another task.

### Objective

Harden static deployment without breaking workers.
...
1. Define CSP for local assets/workers and approved data endpoints
2. Define Referrer, Permissions, X-Content-Type and frame policy
3. Avoid unsafe-inline where practical
4. Document Apache/nginx/Cloudflare examples
5. Keep simulator free of third-party scripts

### Explicit exclusions

- Do not deploy or add remote fonts/scripts

### Required verification

- Validate header configuration and preview expectations
...
- the TASK-093 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
