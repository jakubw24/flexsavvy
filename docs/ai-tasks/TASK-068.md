# TASK-068 — Normalize and match carbon intervals
...
| Suggested commit | `task-068: normalize-and-match-carbon-intervals` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-068 only**. Do not start another task.

### Objective

Align intensities to exact UTC starts.
...
1. Normalize to gCO2/kWh
2. Match exact UTC starts
3. Detect duplicate/missing
4. Never fill silently
5. Return coverage/warnings
6. Handle DST via UTC

### Explicit exclusions

- Do not calculate household emissions
- Do not block monetary simulation

### Required verification

- Test normal, missing, duplicate, unsorted, spring and autumn
...
- the TASK-068 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
