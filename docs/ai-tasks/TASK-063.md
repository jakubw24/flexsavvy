# TASK-063 — Implement grid-charging and export restrictions
...
| Suggested commit | `task-063: implement-grid-charging-and-export-restrictions` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-063 only**. Do not start another task.

### Objective

Constrain battery actions by scenario policy.
...
1. If grid charging disabled, charge only from available export
2. If battery export disabled, discharge only to household demand
3. Allow arbitrage only when flags permit
4. Filter actions before cost
5. Warn when only idle remains

### Explicit exclusions

- Do not infer hardware capability
- Do not bypass physical limits

### Required verification

- Test all four flag combinations with load/export
...
- the TASK-063 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
