# TASK-061 — Implement single-horizon dynamic programming
...
| Suggested commit | `task-061: implement-single-horizon-dynamic-programming` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-061 only**. Do not start another task.

### Objective

Find the least-cost valid path over a fixed horizon.
...
1. Use DP over interval and SOC
2. Track predecessors/actions
3. Include all costs
4. Use deterministic ties
5. Return no-solution diagnostics
6. Keep inputs immutable

### Explicit exclusions

- Do not add terminal constraint, rolling horizon or worker

### Required verification

- Compare one-, two- and small multi-interval cases with manual enumeration
...
- the TASK-061 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
