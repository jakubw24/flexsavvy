# TASK-072 — Implement validation, navigation and progress
...
| Suggested commit | `task-072: implement-validation-navigation-and-progress` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-072 only**. Do not start another task.

### Objective

Control movement through the wizard accessibly.
...
1. Define 11 ordered steps
2. Implement Back/Continue
3. Disable Continue on invalid required input
4. Allow back navigation
5. Show semantic progress/current step
6. Move focus to new heading
7. Prevent skipping invalid steps

### Explicit exclusions

- Do not build step forms or run simulation

### Required verification

- Test keyboard navigation, disabled states, focus and back editing
...
- the TASK-072 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
