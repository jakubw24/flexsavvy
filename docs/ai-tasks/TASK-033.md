# TASK-033 — Implement flat tariff resolution

| Field | Value |
|---|---|
| Phase | 6 — Tariffs and billing |
| Parent milestone | Milestones 7–9 |
| Risk | critical |
| Network policy | `none` |
| Dependencies | TASK-018, TASK-032 |
| Suggested commit | `task-033: implement-flat-tariff-resolution` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-033 only**. Do not start another task.

### Objective

Resolve one deterministic VAT-inclusive flat import rate per eligible interval.
...
1. Validate effective-date overlap
2. Resolve by interval start
3. Return rate and source metadata
4. Return structured missing-rate error outside validity
5. Do not round rate

### Explicit exclusions

- Do not calculate cost
- Do not implement TOU

### Required verification

- Test validity boundaries and out-of-range intervals
...
- the TASK-033 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
