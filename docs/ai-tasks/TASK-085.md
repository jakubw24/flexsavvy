# TASK-085 — Audit results accessibility, responsiveness and reconciliation
...
| Risk | critical |
| Network policy | `none` |
| Dependencies | TASK-080, TASK-081, TASK-082, TASK-083, TASK-084 |
| Suggested commit | `task-085: audit-results-accessibility-responsiveness-and-reconciliation` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-085 only**. Do not start another task.

### Objective

Verify every result surface and numeric identity.
...
1. Audit charts for text/keyboard
2. Audit tables phone/desktop
3. Create summary, waterfall, tables, schedules and intervals
4. Check rounding
5. Check warning/confidence rules
6. Add semantic Playwright assertions
7. Repair results-only defects

### Explicit exclusions

- Do not change engine formulas without isolated proven defect

### Required verification

- Run representative full result scenarios and all checks
...
- the TASK-085 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
