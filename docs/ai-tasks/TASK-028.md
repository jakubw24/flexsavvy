# TASK-028 — Implement multiple-device and replacement resolution

| Field | Value |
|---|---|
| Phase | 5 — Adapters and quality |
| Parent milestone | Milestone 5 |
| Risk | critical |
| Network policy | `none` |
| Dependencies | TASK-027 |
| Suggested commit | `task-028: implement-multiple-device-and-replacement-resolution` |

## Complete prompt

You are Qwen3.6-27B acting as the senior engineer for the FlexSavvy repository.

Execute **TASK-028 only**. Do not start another task.

### Objective

Require explicit choices for multiple meters and overlaps.
...
1. Classify non-overlap as possible replacement
2. Classify overlap as simultaneous meters or duplicates
3. Support select one, combine non-overlap, explicitly sum simultaneous or cancel
4. Never sum same-device duplicates
5. Record transformation and warnings

### Explicit exclusions

- Do not infer household topology
- Do not silently choose

### Required verification

- Test replacement, overlap, same-device duplicate, explicit sum and cancel
...
- the TASK-028 row in `docs/AI_TASK_INDEX.md` is changed from `TODO` to `DONE`;
